...
1.upto(10) do |i|
  ...
  category.products.create!(name: "product #{i}", price_in_sgd: rand(1..100)) if i.even?

  1.upto(2) do |j|
    sub_category = category.children.create!(name: "sub_category #{i}.#{j}")
    sub_category.products.create!(name: "product #{i}.#{j}", price_in_sgd: rand(1..100)) if j.even?
